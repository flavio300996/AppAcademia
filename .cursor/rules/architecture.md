...
- **SEMPRE** que for criar um commit, escolha o tipo correto (`feat`, `fix`, `docs`, `refactor`, `chore`, etc.) e escreva o resumo curto e descritivo em **inglês e português**, no formato `<type>: <description in English> / <descrição em português>`. Exemplo: `feat: add workout session start endpoint / adiciona endpoint para iniciar sessão de treino`.
...
- **SEMPRE** crie as rotas em **arquivos separados** dentro de @src/routes (ex.: `workout-plan.ts`, `workout-session.ts`), **exceto** quando já existir um arquivo de rota diretamente relacionado ao que foi pedido — nesse caso, reutilize o arquivo existente.
- **SEMPRE** utilize IDs no formato **UUID** em todos os parâmetros de rota (ex.: `/:workoutPlanId`, `/:workoutDayId`, `/:sessionId`), e reflita isso nos schemas Zod com `z.string().uuid()`.
...
- **Requisito obrigatório:** para qualquer campo que represente dia da semana, **SEMPRE** utilize `z.enum(WeekDay)` importado de `../generated/prisma/enums.js` (em @src/schemas e em qualquer validação Zod). **NUNCA** use `z.string()` ou literais para representar WeekDay.
...
- **SEMPRE** inclua `tags` e `summary` no schema de **todas** as rotas para documentação no Swagger/OpenAPI (como em `workoutPlanRoutes` e `workoutSessionRoutes`).
